package RAST_SDK::RAST_SDKServer;
...
sub call_method {
    ...
    my $ctx = RAST_SDK::RAST_SDKServerContext->new($self->{loggers}->{userlog},
...
{
    # Service RAST_SDK requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for RAST_SDK but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = RAST_SDK::RAST_SDKServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service RAST_SDK.");
    }
...
package RAST_SDK::RAST_SDKServerContext;

use strict;

=head1 NAME

RAST_SDK::RAST_SDKServerContext
...
package RAST_SDK::RAST_SDKServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = RAST_SDK::RAST_SDKServer->new(
