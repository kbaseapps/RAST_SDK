...
lives_ok {
    ...
    my $genome_gbff_name = "Clostridium_botulinum.gbff";
...
#			print Dumper $ftr->{ontology_terms}->{SSO};
			...
			foreach (@roles) {
				...
				print "CODING Selenocysteine-containing =$ftr->{id}\n";
				$seleno++;
			}
		}
    }
...
#			print Dumper $ftr->{ontology_terms}->{SSO};
			...
			foreach (@roles) {
				next unless ($_ =~ /SSO:000009304/);
				print "Selenocysteine-containing $ftr->{id}\n";
				$seleno++;
			}
		}
		if ($ftr->{function} &&  $ftr->{type} =~ /crispr/) {
#			print Dumper $ftr->{ontology_terms};
			print "Found a NON-coding CRISPR $ftr->{function} $ftr->{id}\n";
			$crispr++;
		}
    }
	foreach my $key (keys %types)
...
print "Number of selenocysteine-containing genes = $seleno\n";
